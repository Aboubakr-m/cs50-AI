    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                if (i, j) == cell:
                    continue
                # find all neighbors of cell to add new knowledge
                if 0 <= i < self.height and 0 <= j < self.width:
                    if (i, j) not in self.moves_made and (i, j) not in self.safes:
                        new_knowledge_cells.append((i, j))
                        
        # add a new sentence to the AI's knowledge base
        new_sentence = Sentence(new_knowledge_cells, count)
        ...
        for sentence in self.knowledge:
            if new_sentence.cells and sentence.cells != new_sentence.cells:
                if sentence.cells.issubset(new_sentence.cells):
                    knowledge_inferred.append(Sentence(new_sentence.cells - sentence.cells, new_sentence.count - sentence.count))
                if new_sentence.cells.issubset(sentence.cells):
                    knowledge_inferred.append(Sentence(sentence.cells - new_sentence.cells, sentence.count - new_sentence.count))
        self.knowledge += knowledge_inferred
